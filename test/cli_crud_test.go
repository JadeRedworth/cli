...
func TestSimpleFnRouteUpdateCycle(t *testing.T) {
	...
	h.Fn("routes", "routes", appName1, "myroute", "--image", "foo/duffimage:0.0.1").AssertSuccess()
...
func TestRemovingRouteAnnotation(t *testing.T) {
	...
	defer h.Cleanup()
	appName1 := h.NewAppName()
	h.Fn("create", "routes", appName1, "myroute", "--image", "foo/duffimage:0.0.1", "--annotation", "test=1").AssertSuccess()
...
func TestInvalidAnnotationValue(t *testing.T) {
	...
	h.Fn("create", "routes", appName1, "myroute", "--image", "foo/duffimage:0.0.1", "--annotation", "test=value").AssertSuccess().AssertStderrContains("Unable to parse annotation value 'value'. Annotations values must be valid JSON strings.")
	h.Fn("inspect", "routes", appName1, "myroute").AssertSuccess().AssertStdoutMissingJSONPath([]string{"annotations", "test"})
}
